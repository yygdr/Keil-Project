...
void Usart0_Init(uint32_t bandrate)
{
	...
	gpio_init(GPIOA,GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_9);  //初�?�化PA9
	gpio_init(GPIOA,GPIO_MODE_IN_FLOATING, GPIO_OSPEED_50MHZ, GPIO_PIN_10); //初�?�化PA10
	
	/* reset USART */
	usart_deinit(USART0);                                           //初�?�化           
	/* configure USART baud rate value */                           
	usart_baudrate_set(USART0, bandrate);                           //设置波特�?
	/* configure USART parity function */
	usart_parity_config(USART0, USART_PM_NONE);                     //设置优先�?
	/* configure USART word length */
	usart_word_length_set(USART0, USART_WL_8BIT);                   //配置传输字长
	/* configure USART stop bit length */
	usart_stop_bit_set(USART0, USART_STB_1BIT);                     //usart停�??位位�?
	...
	nvic_irq_enable(USART0_IRQn,0,0);                               //打开�?�?
	usart_interrupt_enable(USART0, USART_INT_IDLE);                 //空闲�?�?
...
void DMA_Init(void)
{
	...
	dma_init_struct.memory_addr = (uint32_t)U0_RX_BUFFER;          //�?的地址
	dma_init_struct.memory_width = DMA_MEMORY_WIDTH_8BIT;          //接收字节位数
	dma_init_struct.number = U0_RX_MAX + 1;                        //传输字节�?
	dma_init_struct.priority = DMA_PRIORITY_HIGH;                  //优先�?
	dma_init_struct.periph_inc = DMA_PERIPH_INCREASE_DISABLE;      //外�?�地址不�??
	dma_init_struct.memory_inc = DMA_MEMORY_INCREASE_ENABLE;       //接收地址增加
	dma_init_struct.direction = DMA_PERIPHERAL_TO_MEMORY;          //方向

	
	dma_init(DMA0, DMA_CH4, &dma_init_struct);  //???CH4   DMA初�?�化
	
	/* disable DMA circulation mode */
	dma_circulation_disable(DMA0, DMA_CH4);     //关闭DMA�?�?模式？？�?
...
void u0_printf(char *format,...)
{
	...
	{
		j=usart_flag_get(USART0, USART_FLAG_TBE);
		while(usart_flag_get(USART0, USART_FLAG_TBE) != 1);//1:发送缓冲区为空 0:有数�?
